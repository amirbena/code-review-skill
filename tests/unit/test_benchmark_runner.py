...
test-only reference runner (tests/reference/benchmark_runner.py); this
...
class ReferenceModuleTests(unittest.TestCase):
    def test_module_is_declared_test_only(self) -> None:
        head = (REPO_ROOT / "tests" / "reference" / "benchmark_runner.py").read_text(
            encoding="utf-8"
        )[:600]
    ...
    def test_runner_consumes_the_single_fixture_validator(self) -> None:
        raw = (REPO_ROOT / "tests" / "reference" / "benchmark_runner.py").read_text(
            encoding="utf-8"
        )
