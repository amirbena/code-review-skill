...
the single reference counter (tests/reference/benchmark_metrics.py), which
delegates every pairwise decision to the single reference matcher
(tests/reference/benchmark_match.py, #54) — this module never defines a
...
from tests.reference import benchmark_metrics as bmet
...
class ReferenceModuleTests(unittest.TestCase):
    def test_module_is_declared_test_only(self) -> None:
        head = (REPO_ROOT / "tests" / "reference" / "benchmark_metrics.py").read_text(encoding="utf-8")[:700]
    ...
    def test_counter_consumes_the_single_reference_matcher(self) -> None:
        raw = (REPO_ROOT / "tests" / "reference" / "benchmark_metrics.py").read_text(encoding="utf-8")
