#!/usr/bin/env python3
"""Safety and ordering contract for the main/release-lifecycle automation
(issue #168 lifecycle split).

`release-publish.yml` is the authoritative, write-capable path: it is
triggered only by `push` to `main` and, for recovery, `workflow_dispatch`
— never by `pull_request` — and owns the `plan` and `publish` jobs. The
read-only `plan` job derives the version with `auto-release-plan` — there
is no workflow version input — and is itself the authoritative
...
merely a version calculator. The `publish` job runs only when `plan`
reports `should_release == 'true'`, so a merge that ships nothing
releasable never starts a write-capable job or the `release` Environment
...
class TriggerTests(unittest.TestCase):
    ...
    def test_triggered_only_by_push_and_workflow_dispatch(self) -> None:
        on = _on(self.data)
...
class JobTopologyTests(unittest.TestCase):
    """The main/release workflow owns exactly `plan` and `publish` — and
    never `release-gate` or `package`, which stay physically confined to
    the PR-triggered release-worthiness.yml."""
    ...
    def test_does_not_define_package(self) -> None:
        self.assertNotIn("package", self.jobs)

    def test_does_not_reference_pr_body_or_pr_number(self) -> None:
        raw = WORKFLOW.read_text(encoding="utf-8")
        self.assertNotIn("github.event.pull_request", raw)
...
class PlanJobTests(unittest.TestCase):
    ...
    def test_checks_out_main_explicitly(self) -> None:
        checkout = next(
...
class ExtractedHelperWiringTests(unittest.TestCase):
    ...
    def test_subcommand_step_runs_after_checkout_and_python_setup(self) -> None:
        ...
        call = _step_index(steps, "Resolve the authenticated release App bot identity")
    ...
    def test_both_jobs_agree_on_the_helper_never_reimplementing_it(self) -> None:
