...
class CanonicalOwnerTests(unittest.TestCase):
    ...
    def test_review_scope_routes_without_restating(self) -> None:
        ...
        self.assertNotIn("ownership/coordination assumptions", t)
    ...
    def test_policy_indexed_in_readme(self) -> None:
        ...
        self.assertIn("distributed-systems-deepening.md", raw)

    def test_policy_indexed_in_package_manifest(self) -> None:
        manifest = json.loads(PACKAGE_MANIFEST.read_text(encoding="utf-8"))
        sources = json.dumps(manifest)
        self.assertIn(
            "shared/policies/distributed-systems-deepening.md", sources
        )


class BaseObligationUnconditionalTests(unittest.TestCase):
    """The capability never decides whether a concurrency/distributed-
    system concern is considered at all — #211's base pass owns that,
    unconditionally."""
...
class ActivationTests(unittest.TestCase):
    ...
    def test_two_part_activation_condition(self) -> None:
        self.assertIn(
            ...
            "identified a materially implicated Concurrency / "
            "distributed-system semantics dimension",
            self.text,
        )
        self.assertIn(
