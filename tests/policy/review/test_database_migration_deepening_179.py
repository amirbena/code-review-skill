...
class CanonicalOwnerTests(unittest.TestCase):
    ...
    def test_review_scope_routes_without_restating(self) -> None:
        ...
        self.assertIn(
            "[database-migration-deepening.md](database-migration-"
            'deepening.md) per the "Domain-specific deepening pass" '
            "below",
            t,
        )
        # review-scope.md must not restate the capability's worked
        # examples or concern-area catalog
        self.assertNotIn("expand/contract sequencing", t)
        self.assertNotIn("old/new application coexistence", t)
    ...
    def test_policy_indexed_in_readme(self) -> None:
        ...
        self.assertIn("database-migration-deepening.md", raw)

    def test_policy_indexed_in_package_manifest(self) -> None:
        manifest = json.loads(PACKAGE_MANIFEST.read_text(encoding="utf-8"))
        sources = json.dumps(manifest)
        self.assertIn(
            "shared/policies/database-migration-deepening.md", sources
        )


class BaseObligationUnconditionalTests(unittest.TestCase):
    """The capability never decides whether a data/persistence concern is
...
class ActivationTests(unittest.TestCase):
    ...
    that deeper tracing is warranted — never a file-type/path/tool
    router."""
    ...
    def test_two_part_activation_condition(self) -> None:
        self.assertIn(
            ...
            "identified a materially implicated Data / persistence "
            "dimension",
            self.text,
        )
        self.assertIn(
...
class ConcernAreaTests(unittest.TestCase):
    ...
    def test_all_nine_concern_areas_present(self) -> None:
        for phrase in (
