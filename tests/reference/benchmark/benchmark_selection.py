...
``scripts/benchmark/select_benchmark_cases.py``, exactly like
``benchmark_taxonomy.py`` is consumed by
``scripts/benchmark/build_benchmark_index.py``.
...
# Policy relevance bands (docs/benchmark/selection.md §2).
...
# Golden threshold: Selection Coverage >= 60% (docs/benchmark/selection.md §3).
SELECTION_COVERAGE_THRESHOLD: float = 60.0

# Top-K bound: a small default, overridable per run within a fixed,
# non-configurable ceiling (docs/benchmark/selection.md §4).
K_DEFAULT = 6
K_MAX = 12
...
def build_explainability(result: dict[str, Any]) -> dict[str, Any]:
    ...
    JSON object per run described by docs/benchmark/selection.md §5 —
