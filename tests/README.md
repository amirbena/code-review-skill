...
| Directory | Contents |
|---|---|
| `reference/benchmark/` (relocated to [`runtime_platform/benchmark/reference/`](../runtime_platform/benchmark/README.md) by #457) | Test-only reference implementations of the benchmark pipeline: fixture validation, the run-to-run report, the expected-vs-produced matcher, and the missed/incorrect, severity-accuracy, and duplicate-noise metrics built on it. |
...
| `support/` | Shared test infrastructure: `pr_simulation.py` (local bare-repo PR harness), `paths.py` (the one canonical `REPO_ROOT`), and `benchmark_runtime.py` (the shared gate for tests that need the real benchmark review runtime; `BENCHMARK_REQUIRE_RUNTIME=1`/`true`/`yes` makes a missing runtime an error instead of a skip); `benchmark_publisher_fakes.py` holds the sealed-record builders and in-memory GitHub world the publication-CLI tests (#471) run against. |
...
| `integration/packaging/` | End-to-end packaging-boundary guard: manifest/path-safety, script parity, hidden-runtime-dependency + disclaimer prose, the built local/GitHub archive contents, and `test_distribution_consumer_install.py` (#511: the published distribution tree is discoverable and installable through the `skills` CLI with self-contained copies; the `npx` tests run only with `DISTRIBUTION_INSTALL_CHECK=1`) (`_shared.py` holds the shared paths, manifest helpers, and the reference-module list). |
| `integration/release/` | Coverage that builds the packaged archives to exercise the release-worthiness PR boundary. |
| `policy/benchmark/` | Prose / documentation-contract checks for the benchmark contract docs (`runtime_platform/benchmark/**`) and the corpus data that stays under `docs/benchmark/corpus/`. |
...
| `policy/review/` | Prose / documentation-contract checks over `shared/policies/`, `shared/templates/`, and each Skill's review-related files. Sub-packages mirror `unit/review/`'s ownership clusters where the same boundary applies (#293): `findings/`, `stateful_review/`, `specialist_depth/`, `root_cause/`, `repository_intelligence/`, `stacked_pr/`, plus `presentation/` (human-first/senior-voice output, output-format separation, and the #223/#275 output-tightening options — a policy-only cluster with no `unit/review/` counterpart). Everything else — core `review-scope.md` wiring, context/evidence, PR context, requirement coverage, and other modules with no second clustering member — stays directly under `policy/review/`. |
| `repository/` | Repository-hygiene checks (e.g. `.gitignore`, and `scripts/` staying subsystem-owned per #292). Unchanged by the #217 reorganization. |
