...
| `support/` | Shared test infrastructure: `pr_simulation.py` (local bare-repo PR harness), `paths.py` (the one canonical `REPO_ROOT`), and `benchmark_runtime.py` (the shared gate for tests that need the real benchmark review runtime; `BENCHMARK_REQUIRE_RUNTIME=1`/`true`/`yes` makes a missing runtime an error instead of a skip). |
