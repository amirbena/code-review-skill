...
| --- | --- | --- | --- | --- |
| Validate repository | [`workflows/validate.yml`](workflows/validate.yml) | `pull_request`, `push` to `main` | `route`: run the trusted router (`scripts/validation/ci_test_route.py`) from the PR base SHA to pick the FAST or FULL tier (FULL by default, on any error, and on every push to `main`). `test` (required): set up Python 3.13, validate both Skills' metadata (`scripts/validation/validate-skill-metadata.py`), build and spec-validate the Skill trees, then run `python -m unittest discover -s tests -t .` (FULL) or the same discovery minus `tests.integration.*` (FAST). Skill-tree shell/PowerShell parity on both tiers | Read-only (`contents: read`) |
