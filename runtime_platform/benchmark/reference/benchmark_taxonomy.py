...
Mirrors ``runtime_platform/benchmark/taxonomy.md``, the canonical, alias-free contract
...
- :func:`validate_taxonomy` — **fail-closed**, for hand-authored corpus
  case metadata (``runtime_platform/benchmark/fixture-format.md`` §10). A human
...
CAPABILITY_BY_CORPUS_DIRECTORY: dict[str, str] = {
    ...
    "finding-placement": "finding-placement",
...
# (runtime_platform/benchmark/scripts/) without pulling in the fixture parser; a policy test
...
# (runtime_platform/benchmark/scripts/benchmark_ci_classifier.py, retired by #420). That
...
_RUNTIME_ADAPTER_EXACT_FILES: frozenset[str] = frozenset(
    {
        "runtime_platform/benchmark/scripts/run_benchmark.py",
...
# (which live under runtime_platform/benchmark/scripts/) are not shadowed by the broader
