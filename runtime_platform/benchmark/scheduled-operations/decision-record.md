...
The benchmark is **never scheduled or executed through GitHub Actions**.
Execution happens outside GitHub, produces a sealed canonical result, and
only then does GitHub enter — through a publication-only identity.

```text
EXECUTION SIDE  (outside GitHub; holds no GitHub write credential)
  Claude Cloud Routine  |  admissible external scheduler   (scheduler adapter)
    ...
    → handoff                      (durable; the only thing that leaves)
─────────────────── one-way publication boundary ───────────────────
PUBLICATION SIDE  (holds the App key; deterministic; never judges)
  Publisher → benchmark-publication GitHub App
...
REPOSITORY-SIDE VALIDATION  (GitHub Actions, optional, read-only)
  checks published contracts/schemas — never executes or publishes a benchmark
```
...
quality drifted. Full boundary specification:
...
| E16 | The `main` branch has an active repository ruleset; the release App is its sole documented bypass actor. There is no benchmark-publication App. | `gh api repos/amirbena/code-review-skill/rulesets`; [`docs/RELEASE.md`](../../../docs/RELEASE.md) |
...
- **Routines** ([Claude Code docs — routines](https://code.claude.com/docs/en/routines);
  ...
  and no explicit DST behavior — both are **unverified** here.
...
| ID | Decision | Detail |
| --- | --- | --- |
| D1 | **Execution/publication boundary.** Execution, verification and drift evaluation happen outside GitHub and end in a sealed canonical result. GitHub is entered only afterwards, only through the publication App, one-way. | [`execution-publication-boundary.md`](execution-publication-boundary.md) §1–§2 |
| D2 | **Scheduler.** Claude Cloud Routine remains the default scheduler behind a scheduler-adapter boundary; the core stays scheduler-independent. GitHub Actions `schedule:` and Desktop scheduled tasks stay rejected. §2.2 needs an *optional* narrow amendment (an admissibility test) to allow another scheduler, and no amendment for the recommended architecture itself. | [`execution-publication-boundary.md`](execution-publication-boundary.md) §3; [`contract-reconciliation.md`](contract-reconciliation.md) A3 |
...
| D4 | **Handoff.** A run is *verified* only once it is **sealed** into a durable handoff; publication is retried from the handoff and never re-runs the benchmark. Recommended transport: a `claude/`-prefixed staging ref written with the Routine's provider-native checkout transport, so no credential is provisioned into the Routine. Alternative: a write-only external store. | [`execution-publication-boundary.md`](execution-publication-boundary.md) §4 |
| D5 | **Publisher.** A deterministic, non-LLM, repo-owned program that holds the App key. It runs outside the Routine and outside GitHub Actions; the host is provider-owned behind an adapter, and a maintainer-run `--once` mode is the bootstrap and disaster-recovery path. | [`execution-publication-boundary.md`](execution-publication-boundary.md) §5 |
| D6 | **App identity.** A dedicated `benchmark-publication` GitHub App, distinct from the release App (which stays the sole `main` bypass actor), with per-option minimum permissions, phase-scoped one-hour installation tokens, and a closed capability set. | [`execution-publication-boundary.md`](execution-publication-boundary.md) §6–§8 |
...
| D8 | **Failure and recovery.** Idempotency is `run_id` plus markers on GitHub objects; the receipt is derived, never authoritative. A gap-based watchdog inside the publisher detects missed runs without any timezone logic. | [`drift-issue-lifecycle-and-recovery.md`](drift-issue-lifecycle-and-recovery.md) §5–§7 |
| D9 | **GitHub Actions role.** Read-only validation of repo-owned published contracts (schemas, manifest, docs) only. No `schedule:`, no benchmark execution, no App key, no publication. | [`execution-publication-boundary.md`](execution-publication-boundary.md) §9 |

### Rejected alternatives (summary)

| Alternative | Rejected because | Where |
| --- | --- | --- |
| GitHub Actions cron running the benchmark | Excluded by this record's constraint, by [#391](https://github.com/amirbena/code-review-skill/issues/391)/[#420](https://github.com/amirbena/code-review-skill/issues/420)/[#431](https://github.com/amirbena/code-review-skill/issues/431), and by the Class 1 boundary. | boundary §3 |
| GitHub Actions as the *publisher* (simplest key custody via secrets) | Puts Actions in the publication path; this record limits Actions to validation. The cost of that constraint is a separately hosted publisher. | boundary §5 |
...
| A second Routine as publisher | An LLM session holding a signing key and reading fire text is the wrong custody boundary. | boundary §5 |
| Maintainer `gh` identity for publication | Benchmark process would hold write authority; identity is not least-privilege or independently revocable. | boundary §6 |
| Actions artifacts as the store | Need a workflow run to create; 90-day default retention; tied to runs. | persistence §3 |
| Issue comments as the store | Editable, length-capped, unschematized, paginated for baseline reads. | persistence §3 |
...
| --- | --- | --- |
| M1 | Handoff transport: `claude/` staging ref (no provisioned credential; the provider-native GitHub identity still exists) or a write-only external store (no GitHub write at all from the Routine; new infrastructure and a secret)? | Staging ref. Choose the external store only if a zero-GitHub-write Routine is a hard requirement. |
| M2 | Publisher host (any maintainer-controlled scheduled runner with a secret store). | Decide in F8's spike; start with the manual `--once` mode. |
...
| Per-option minimum permissions; benchmark runtime holds no write authority | [`execution-publication-boundary.md`](execution-publication-boundary.md) §6–§8 |
