...
| **A14** | [`execution-publication-boundary.md`](execution-publication-boundary.md) §8; [`cloud-routine-integration.md`](../../../docs/benchmark/cloud-routine-integration.md) §4; [`drift-issue-lifecycle-and-recovery.md`](drift-issue-lifecycle-and-recovery.md) §3, §5; the provisioning runbook P6/P7 | Tracking and health issues are locked to collaborators and the App comments on them; markers are recognised by scanning comment bodies with no author check. The App's installation token was refused on a locked issue (`HTTP 403`, #484 P7, recorded in the runbook §4). | Tracking and health issues are unlocked. Marker recognition and lookup of the publisher's own status comment count only comments authored by the publisher identity `<app-slug>[bot]`; a refused App post fails publication closed, with no personal-identity fallback. P7 verifies POST → author → PATCH → DELETE. Landed by #489. | Blocking |
...
A14 was raised after F1 and lands through its own docs-only issue,
[#489](https://github.com/amirbena/code-review-skill/issues/489), approved on
[#466](https://github.com/amirbena/code-review-skill/issues/466#issuecomment-5742792835).
