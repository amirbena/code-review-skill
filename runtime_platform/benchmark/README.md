...
| [`publication-cli.md`](publication-cli.md) | The deterministic publisher's `sweep` command: App-token-only credentials and acting identity, `--once` and `--dry-run`, the handoff file it reads, the ordered validate → persist → reconcile → announce → receipt pass with its refusal gates, publisher-authored marker recognition (A14), lane-aware drift-issue resolution (A9), recurrence links, spam bounds, and failure-table recovery. Retires the in-Routine `GhCliIssueClient` path. | [#471](https://github.com/amirbena/code-review-skill/issues/471) |
