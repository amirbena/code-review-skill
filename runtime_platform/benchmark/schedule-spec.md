...
| `lanes.<lane>.expected_from` | The UTC instant (`YYYY-MM-DDTHH:MM:SSZ`) from which the lane is expected to have run; `null` until activated. It is the watchdog's reference until the lane has a published verified scheduled record, and thereafter a floor: a lane is overdue when `now − max(latest record's finished_at, expected_from) > max_gap_hours`. The maintainer sets it in the manifest when the Routine is enabled ([#475](https://github.com/amirbena/code-review-skill/issues/475)); it is an instant, not a schedule, and is not a provisioning requirement (`--require-provisioned` accepts `null`). |
