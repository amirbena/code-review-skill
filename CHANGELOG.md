...
Entries under `## Unreleased` are generated at release time from the
`Release category:` / `Release entry:` lines of each merged pull request's
description — contributors do not edit this file — and grouped by Keep a
Changelog category (`Added` / `Changed` / `Deprecated` → minor, `Fixed` /
`Security` → patch, `Removed` / `Breaking` → major). The highest category
present determines the next version, which is derived and published
automatically when the change reaches `main`. See
...
- Clarify in `review-stopping-criteria.md` that its decision-affecting coverage concept is distinct from issue #182's observational review-execution telemetry (#430).
...
- Compress the github-pr-review active/passive runbooks by replacing restated policy procedures with delegations to their canonical owning policies, with no change in review behavior (#291).
...
- Generalized the Database/Migration deepening review (#281).

## v1.32.0 — 2026-09-13

### Added

- Add a Database / Migration deepening capability for schema-evolution and migration risk review (#277).
...
- Add the Security deepening capability for evidence-driven trust-boundary investigation (#83) (#270).

## v1.29.0 — 2026-09-13

### Added

- Add the specialist-depth composition contract for adaptive domain-specific review deepening (#82) (#267).
