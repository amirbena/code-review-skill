...
| `.github/workflows/release-worthiness.yml` | 346 | extract | Three jobs (`assess` / `plan` / `publish`) with many multi-line `run: |` shell blocks carrying changelog, version/tag-resolution, and publish logic. Target: move reusable shell logic into `release_lib` subcommands or `scripts/release/` helpers invoked by the workflow; the YAML keeps the job graph, permissions, and gating only. Release automation is CI-only (Linux) — no `.ps1` parity required, stated in the extracted helpers. |
| `scripts/package-skills.ps1` | 354 | reduce duplication | See below — paired with the `.sh`. |
| `scripts/package-skills.sh` | 330 | reduce duplication | The two scripts are intentional cross-platform parity, but the **per-Skill package manifest** (which resources each Skill ships, archive layout, containment rules) is restated in both and echoed by the validator and tests. Target: one declarative manifest (JSON/YAML) both scripts read; each keeps only thin platform-specific archiving. Archive output must stay identical on both platforms. |
