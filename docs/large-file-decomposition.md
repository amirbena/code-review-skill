...
| File | Lines | Tag | Rationale and target shape |
| --- | ---: | --- | --- |
| `shared/templates/finding.md` | 546 | completed | Issue #198 split the field/quality contract (kept in `finding.md`) from the canonical rendering exemplars (full / inline / human-inline / summary-pointer), now in `shared/templates/finding-rendering.md`; the two are cross-linked and both packaged. Subsequent optional-field additions (#118 contextual evidence, #128 runtime validation, #178 confidence) each add one cohesive field section; the file stays the single finding contract. |
...
| `runtime_platform/benchmark/fixture-format.md` | 438 | keep | Canonical benchmark schema with worked examples; `tests/policy/benchmark/test_benchmark_fixture_docs.py` asserts its sections. Keep. |
| `runtime_platform/benchmark/missed-and-incorrect-findings.md` | 351 | keep | Canonical metric contract with asserted structure. Keep. |
...
section), `runtime_platform/benchmark/match-criteria.md` (289),
`runtime_platform/benchmark/severity-accuracy.md` (285),
`runtime_platform/benchmark/duplicate-noise.md` (285),
`skills/local-code-review/policies/pr-context.md` (284),
`runtime_platform/benchmark/regression-report.md` (283).
