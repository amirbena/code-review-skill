...
| **Review result** | The verdict the review produced: `REVIEW CLEAN` / `CHANGES REQUIRED` (local), `Approve` / `Request Changes` / non-mutating `PASSIVE` (PR), or an explicit non-graded outcome (`REVIEW INCOMPLETE`, `JIRA CONTEXT UNRESOLVED`, `NO NEW DELTA`). | Only a **completed, graded** review is a valid prior state to diff from. A non-graded outcome is recorded but never seeds a delta (§7). |
