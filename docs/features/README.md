...
| Review execution telemetry (#182) — observational record of files inspected, symbols expanded, repository-intelligence expansions, runtime validations, partitions, and stage completion | repository-development design work, not a user-facing knob; purely observational and, per [`review-stopping-criteria.md`](../../shared/policies/review-stopping-criteria.md)'s own "Relationship to review execution telemetry" pointer, never decision-affecting — a deliberately different model from that policy's own coverage concept | [`review-telemetry/README.md`](../review-telemetry/README.md) |
| Verdict-consistency boundary (pre-render/pre-publish check that a rendered or submitted decision signal agrees with the mechanically-derived decision) | always on; a mismatch withholds the protected render/publication and reports, never self-corrects or warns-and-continues | [`verdict-consistency.md`](../../shared/policies/verdict-consistency.md) |
...
| Review-base policy compliance | always on; repository-relative (never a hardcoded branch name), fails closed when the repository-resolved review base or the base under review cannot be reliably resolved, and never flags a legitimate stacked-PR parent base — one blocking P0 before implementation findings, not a user-facing knob | [`review-base-policy.md`](../../shared/policies/review-base-policy.md) |
