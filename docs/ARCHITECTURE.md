...
the single source of truth. The one other rewrite is the staged
`SKILL.md`'s frontmatter `version`, stamped from the newest `## vX.Y.Z`
heading in `CHANGELOG.md` so an archive never reports a stale committed
value (see [`RELEASE.md`](RELEASE.md), "Skill archive version").
