...
| [`nightly-history-and-baseline.md`](nightly-history-and-baseline.md) | Scheduling the #415 vehicle's `full` mode on a maintainer-configured Cloud Routine; the `benchmark-history`-branch storage shape, retention/rotation, and first-run bootstrap; and the chosen baseline policy (a pinned, deliberately-refreshed reference) with the rationale for why a naive last-known-good or rolling baseline was set aside. | [#338](https://github.com/amirbena/code-review-skill/issues/338) |
