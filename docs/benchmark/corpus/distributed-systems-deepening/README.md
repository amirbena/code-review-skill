...
| [`distributed-systems-deepening-retry-idempotency-guarded-clean.yaml`](distributed-systems-deepening-retry-idempotency-guarded-clean.yaml) | retry interaction, idempotency already guarded | report **nothing required** — base reasoning already suffices (an optional note on the guard's in-memory durability is also acceptable) | `clean` |
