...
| [`database-migration-deepening-dynamodb-gsi-backfill-missing.yaml`](database-migration-deepening-dynamodb-gsi-backfill-missing.yaml) | DynamoDB GSI added with no backfill for pre-existing items | report one **P1**: `list_orders_by_status` queries the new `status-index` GSI, which DynamoDB never retroactively populates for items written before the index existed, and no backfill or fallback exists (an optional missing-migration-test note is also acceptable) | `changes-required` |
