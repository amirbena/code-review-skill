...
| Human-style summary output | opt-in `human_review_output` (natural language; no flag) renders the returned report's summary in a concise senior-engineer voice; findings / severity / decision unchanged. `human_inline_findings` is normalized for parity but inert (no inline surface) | same opt-in; renders the final review summary comment (and self-review `COMMENT`) concisely. Companion `human_inline_findings` (derived default `explicit_value ?? human_review_output`) renders the inline review comments in the same voice — a severity-keeping heading plus prose in place of `Evidence` / `Impact` / `Fix`; findings / severity / identity / review state / canonical fix-action anchor / `#164`-`#165` fallback / machine-readable status unchanged |
...
| **Human-style summary output** | opt-in `human_review_output` (default off), normalized from natural language only — "make the review shorter and more human", "like a senior engineer", "use concise review comments"; no CLI flag ([`invocation-options.md`](../shared/policies/invocation-options.md)); re-renders only the human-facing body in a concise senior-engineer voice, trailing metadata unchanged. `human_inline_findings` normalized for parity but inert (no inline surface) | same opt-in; re-renders the final review summary comment (and the self-review `COMMENT`) concisely, each referenced finding keeping its P0/P1/P2 label. Companion `human_inline_findings` (derived default `explicit_value ?? human_review_output`; explicit value wins either way) re-voices the **inline comments** to match — a severity-keeping heading plus prose in place of `Evidence` / `Impact` / `Fix`. Findings, severity, finding identity, dedup, verdict, GitHub review state, canonical fix/action anchor, `#164`/`#165` body fallback, and machine-readable status are identical to mode-off; only the summary wording (and, under `human_inline_findings`, the inline wording) changes |
