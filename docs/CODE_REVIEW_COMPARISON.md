...
| Human-style summary output | opt-in `human_review_output` (natural language; no flag) renders the returned report's summary in a concise senior-engineer voice; findings / severity / decision unchanged. `human_inline_findings` is normalized for parity but inert (no inline surface) | same opt-in; renders the final review summary comment (and self-review `COMMENT`) concisely, and — for any finding rendered in full in the body rather than inline (a passive review's findings, or an active fallback finding with no valid inline anchor) — that finding too. Companion `human_inline_findings` (derived default `explicit_value ?? human_review_output`) is scoped only to the inline review comments, rendered in the same voice — a severity-keeping heading plus prose in place of `Evidence` / `Impact` / `Fix`; findings / severity / identity / review state / canonical fix-action anchor / `#164`-`#165` fallback / machine-readable status unchanged. Publishing a previously produced passive review with no stated presentation asks once which to use |
...
- **Review analysis separate from GitHub mutation authority** — `github-pr-review` runs under one
  canonical publication mode (`PASSIVE` / `SEMI` / `ACTIVE`), defaulting to non-mutating `PASSIVE`;
  an explicit `ACTIVE` request is itself sufficient authorization to publish `APPROVE` /
  `REQUEST_CHANGES`, subject to genuine reviewer independence and GitHub event permission; a
  verdict is not, by itself, a GitHub event, and approval is not merge authority
...
| **Publication ordering** | one returned document; the human-facing summary is last, trailing machine metadata subordinate | `final review comment == last publication event`: inline comments + review body + event submitted as one batched review; any optional machine-readable status published **before** that submission; nothing review-owned published or edited after the final summary — identical whether or not `human_review_output` is on ([`review-output.md`](../skills/github-pr-review/policies/review-output.md), "Submission ordering") |
| **Human-style summary output** | opt-in `human_review_output` (default off), normalized from natural language only — "make the review shorter and more human", "like a senior engineer", "use concise review comments", "senior review"; no CLI flag ([`invocation-options.md`](../shared/policies/invocation-options.md)); re-renders only the human-facing body in a concise senior-engineer voice, trailing metadata unchanged. `human_inline_findings` normalized for parity but inert (no inline surface) | same opt-in; re-renders the final review summary comment (and the self-review `COMMENT`) concisely, and any finding rendered in full in the body rather than inline (a passive review's findings, or an active fallback finding with no valid inline anchor), each referenced finding keeping its P0/P1/P2 label. Companion `human_inline_findings` (derived default `explicit_value ?? human_review_output`; explicit value wins either way) is scoped only to the **inline comments**, re-voicing them to match — a severity-keeping heading plus prose in place of `Evidence` / `Impact` / `Fix`. Findings, severity, finding identity, dedup, verdict, GitHub review state, canonical fix/action anchor, placement, `#164`/`#165` body fallback, and machine-readable status are identical to mode-off; only the summary wording, a body/fallback finding's wording, and (under `human_inline_findings`) the inline wording changes. Publishing a previously produced passive review with no stated presentation asks once which to use rather than silently defaulting to structured |
...
| **Mutation / read-only boundaries** | read-only; never edits, commits, pushes, or performs any GitHub mutation — even with a PR reference | read-only on Git and implementation; the repository-backed checkout is an isolated throwaway clone, never the target repo, and read-only; GitHub mutation only in `ACTIVE` publication mode; non-mutating `PASSIVE` by default, `APPROVE` submitted only when the explicit `ACTIVE` request clears reviewer independence and GitHub event permission; max positive action is **Approve**; never merges (merge authority never inferred from a verdict or an approval); never executes target-repo code |
