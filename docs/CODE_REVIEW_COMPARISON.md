...
| **Remediation guidance** | concise direction by default; explicit `include_fix_prompt=true` may add one coding-agent-ready prompt per qualifying root-cause finding; output-only and still read-only | concise reviewer-facing recommended direction only; no local-style full implementation prompt; severity, verdict, and mutation boundaries unchanged |
