...
| **Human-style summary output** | opt-in `human_review_output` (default off), normalized from natural language only — "make the review shorter and more human", "like a senior engineer", "use concise review comments"; no CLI flag ([`invocation-options.md`](../shared/policies/invocation-options.md)); re-renders only the human-facing body in a concise senior-engineer voice, trailing metadata unchanged | same opt-in; re-renders the final review summary comment (and the self-review `COMMENT`) concisely, each referenced finding keeping its P0/P1/P2 label; findings, severity, dedup, verdict, GitHub review state, inline comments, and machine-readable status are byte-identical to mode-off |
