...
Reviewer ownership and review-mode resolution — the self-review guard
("Self-review capability") and delta-vs-full mode resolution ("Reviewer
ownership and delta re-review") — are authoritative and resolve first,
before this reasoning flow begins. Once this invocation's scope is
established, review reasoning proceeds:
...
complete reasoning boundary — see "Logical Cohort Review" and "Code
Impact / Dependency Analysis" below. For a delta re-review, this
reasoning flow applies to the reviewed delta and any surrounding context
required to validate it, per "Reviewer ownership and delta re-review";
it does not change the delta boundary itself.
...
previously reviewed SHA equals the current PR HEAD — see "Reviewer
ownership and delta re-review" above. No new review is submitted in this
case.
...
This check runs after the self-review guard above (which remains
authoritative and terminates first, before this check ever runs) and
before this invocation's review scope is established, so the outcome
determines *how much* of the PR this invocation reviews — never
*whether* it reviews at all, and never a bypass of the self-review
guard.
...
described in "Complete PR scope and pagination" below — that requirement
governs a normal full review's scope, not a bounded delta re-review's.
...
NEW DELTA` outcome in "Final decision" below — this Skill does not
resubmit a duplicate review for a HEAD this same reviewer already
completed.
...
"Existing review awareness" below). The current reviewer does not
inherit another reviewer's judgment: previous findings and conclusions
are not already-verified facts merely because a prior review exists.
Independently inspect the full relevant PR change and reach independent
conclusions; prior reviews may inform investigation but never substitute
for it.
...
"Existing review awareness" below) remain authoritative.
